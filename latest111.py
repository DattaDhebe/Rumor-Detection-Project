...
def fcode():
    ...
    #plt.show()
    plt.savefig('statics/images/train_test.png')
    ...
    plt.ylabel('loss')
    plt.legend(['train', 'test'], loc='upper right')
    plt.savefig('statics/images/epoch_loss.png')
    #plt.show()
    print("\nTesting---------")
    ...
    plt.ylabel('Actual')
    #plt.savefig('statics/images/prediction.png')
    ...
    path = os.path.join("check/E699")
    TidList = listdir(path)

    TweetList = []
    t_list = []
    for TweetId in TidList:
        ft = open(os.path.join("check/E699", TweetId), "r")
        file1 = json.load(ft)
        tweet1 = file1['text']
        text1 = remove_pattern(tweet1, "@[\w]*")
        text2 = remove_link(text1)
    ...
    mylist = list(dict.fromkeys(t_list))
    print(mylist)
    ...
    print(mylist)
    print("respective count")
    print(counter1)

    y_pos = np.arange(len(mylist))
    plt.plot(y_pos, counter1)
    plt.xticks(y_pos, mylist, rotation=90)
    plt.xlabel('Dates of propogation of tweet')
    plt.ylabel('Number of tweets')
    # plt.show()
    # fig = plt.figure()

    # graph = fig.add_subplot(111)
    # graph.plot_date(mylist, counter1)


    plt.savefig('statics/images/gr.png')
    ...
    X_test = np.array(totalData)
    y_pred1 = model.predict_classes(X_test)

    if y_pred1[0] == 0:
        print("Event is not a Rumour")
        return "Event is not a Rumour"
    else:
        print("Event is a Rumour")
        return "Event is a Rumour"
